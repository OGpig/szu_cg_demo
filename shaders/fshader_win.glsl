...
void main()
{
	...
	// 阴影
		...
		// 使用固定的透明度控制阴影颜色的深浅
    float shadowOpacity = 0.5; // 阴影的透明度，可根据需求调整

    // 计算光照方向与法向量之间的夹角，以得到阴影深度
    vec3 L = normalize(light.position - position);  // 光源方向
    vec3 N = normalize(normal);                     // 表面法向量
    float shadowIntensity = max(dot(L, N), 0.3);    // 确保最小阴影强度不为零

    // 将阴影颜色设置为黑色，但加入深浅变化
    vec3 shadowColor = vec3(0.0, 0.0, 0.0) * shadowIntensity;
    fColor = vec4(shadowColor, shadowOpacity);
	}
	else if (type == 0) {
		// 将顶点坐标、光源坐标和法向量转换到相机坐标系
		vec3 pos = position - eye_position;
